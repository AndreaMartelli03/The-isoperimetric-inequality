...
The proof is basically just a computation: test the function \(u(x) = H (\nu(x) \cdot x)-n+1\) in \eqref{eq: stability inequality}. 

A.D. Alexandrov, with the celebrated \emph{moving plane method} (i.e., the maximum principle), proved that actually spheres are the only \emph{critical points} of the area functional with respect to variations with fixed volume.
...
However, this approach encounters an insurmountable limit in higher dimension: we are implicitly assuming that the category of sets with smooth boundary contains the minimizers of the isoperimetric problem. This is not obvious at all. Indeed, for dimension \(n \ge 8\) there exist area minimizing hypersurfaces that present singularities. This phenomena was first discovered by J. Simons in \cite{Simons_MinimalVarieties1968}, where he proved that the cone (which has been named after him)
\[
    C \coloneq \{ (x,y) \in \R^4 \times \R^4 \colon |x|=|y|\}
\]
is a stable minimal hypersurface away from 0, i.e., \(H=0\) and the inequality \eqref{eq: stability inequality} holds with every compact supported function \(u\) on \(C\setminus\{0\}\). One year later, Bombieri, De Giorgi and Giusti proved that \(C\) is actually area minimizing \cite{BombieriDeGiorgiGiusti_Bernstein1969} and found examples for every \(n \ge 8\).

In general, an area minimizing ‘‘hypersurface'' \(\Sigma \subset \R^n\) (here, we should use the language of currents and substitute \(\Sigma\) with an \((n-1)\)-integral current) is smooth if \(n \le 7\), while if \(n \ge 8\) its singular set has Hausdorff dimension at most \((n-1)-7\). This result has been proved with the hard work of many great mathematicians such as Fleming and De Giorgi (\(n=3\), \cite{Fleming, DeGiorgi1965}), Almgren (\(n=4\), \cite{Almgren_InteriorRegularity}), Simons (\(5\le n \le 7\), \cite{Simons_MinimalVarieties1968}) and Federer (\(n \ge 8\), \cite{Federer1970}). 