...
\begin{rmk}
    \emph{A posteriori}, assuming that \(\Sigma\) is connected in the isoperimetric problem \eqref{eq: isoperimetric problem} is not restrictive. In fact assume that \(\Sigma = \de \Omega\) is not connected. Without loss of generality, we can assume that \(\Omega\) is connected (otherwise consider separately each connected component). Then each connected component is CMC-stable, hence by Theorem~\ref{thm: Barbosa-do Carmo} they are spheres. By Jordan's separation theorem, each sphere is also a boundary of a ball, so \(\Omega\) must be obtained by removing some disjoint balls \(B_1,\dots, B_k\) (of radii \(r_1,\dots,r_k\)) from an initial big ball \(B_0\) (of radius \(r_0\)). Hence, 
    \[
    \begin{aligned}
        \Haus^{n-1}(\Sigma) &= \sum_{j=0}^k \Haus^{n-1}(\de B_j) = n \omega_n\sum_{j=0}^k r_j^{n-1} \\
        &\ge n\omega_n r_0^n=n\omega_n^{1/n}|B_0|^{\frac{n-1}{n}}\\
        &\ge n\omega_n^{1/n}|\Omega|^{\frac{n-1}{n}}.
    \end{aligned}
    \]
\end{rmk}
