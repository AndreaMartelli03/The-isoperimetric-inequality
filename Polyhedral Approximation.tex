...
\end{defn}
\begin{rmk}
    It's not hard to prove that any function \(u \in C^1_c(\R^n)\) can be approximated by compactly supported finitely piecewise affine functions in the \(C^1\)-norm.
\end{rmk}
...
\begin{lemma}[Approximation by finitely piecewise affine functions]\label{lemma: approximation by finitely piecewise affine functions}
    For every \(u \in BV(\R^n)\) there exists a sequence \(u_h \colon \R^n \to \R\) of finitely piecewise affine functions such that \(u_h \to u\) in the BV-strict sense, that is, \(u_h \to u\) in \(L^1(\R^n)\) and
    \[
...
\end{equation}
where \(A\triangle B = (A\setminus B) \cup (B \setminus A)\) is the symmetric difference. This is the usual interpretation of the left hand side as the area between the graph of \(u\) and the graph of \(v\) over \(E\). Indeed, consider separately \(E_+=E \cap \{u>v\}\) and \(E_-=E \cap \{u<v\}\). By Fubini's theorem,
\begin{align*}
    \int_{E^+} |u-v| \dif x &= \int_{E_+} \Leb^1((v(x),u(x))) \dif x \\
...
\begin{proof}[Proof of Theorem~\ref{thm: polyhedral approximation}]
    \emph{Step 1.} By Lemma~\ref{lemma: approximation by finitely piecewise affine functions}, there is a sequence of piecewise affine functions \(u_h \colon \R^n \to \R\) such that \(u_h \to \chi_E\) in \(L^1(\R^n)\) and 
    \begin{equation}\label{eq: |Du_h|(R^n) -> P(E)}
        \lim_{h \to \infty} \int_{\R^n} |\nabla u_h| \dif x = P(E).
    \end{equation}

    For every \(h \in \N\) and \(t \in (0,1)\) consider the set
    \[
        E_h^t \coloneq \{u_h >t\}.
    \]
