...
\end{rmk}
\begin{rmk}
    It's not hard to prove that any function \(u \in C^1_c(\R^n)\) can be approximated by \emph{compactly supported finitely piecewise affine functions} in the \(C^1\)-norm. Indeed, one could proceed as follows: take \(\eps>0\) and subdivide \(\R^n\) in cubes of side \(2\eps\) and with faces parallel to the coordinates planes. Then interpolate the values assumed by \(u\) on the vertices of those cubes with an affine function on each cube, obtaining a finitely piecewise affine function \(u_\eps \in C^0_c(\R^n)\), whose gradient \(\nabla u_\eps\) is defined away from the boundaries of those cubes and constant in the interior of each cube. It's easy to check that \(u_\eps \to u\) and \(\nabla u_\eps \to \nabla u\) in \(L^\infty(\R^n)\).  
\end{rmk}
