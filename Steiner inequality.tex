...
\begin{proof}
    ...
    Now, assume that \(P(E)=P(E^s)\) and let us prove that \(E_z\) is equivalent to an interval for a.e. \(z \in \R^{n-1}\). Moreover, since \(E_h \to E\), we have that
    \begin{equation*}
        G_h \to G.
    \end{equation*}
    ...
    \end{aligned}
    \end{equation*}
    having used that for every \(z \in G_h \setminus D_h\) the slice \((E_h)_z\) is equivalent to a bounded interval, and the 1-dimensional isoperimetric inequality, i.e., that \(P(E_z) \ge 2\) for a.e. \(z \in \R^{n-1}\). Then, \(P(E_z)=2\) for a.e. \(z \in \R^{n-1}\), so, by the characterization of the equality in the 1-dimensional case, \(E_z\) is an interval for a.e. \(z \in \R^{n-1}\). 
\end{proof}
