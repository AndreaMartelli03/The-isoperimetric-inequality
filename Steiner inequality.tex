Let us recall the definition of Steiner symmetrization. Write \(\R^n = \R^{n-1}\times \R\) with coordinates \(x=(z,t)\), \(z \in \R^{n-1}\), \(t=x_n \in \R\). For every \(E \subset \R^n\) and \(z \in \R^{n-1}\), consider the \emph{vertical \(z\)-slice}
\[
    E_z \coloneq \{ t \in \R \colon (z,t) \in E\} \subset \R.
\]
The \emph{Steiner symmetrization of \(E\) with respect to the direction \(e_n\)} is the set
\[
    S_{e_n}(E) \coloneq \left\{ (z,t)\in \R^{n-1}\times \R \colon |t| \le \frac{1}{2}\Leb^1(E_z)\right\}.
\]
By Fubini's theorem, \(|S_{e_n}(E)|=|E|\). It is also quite easy to prove that
\[
    \diam(S_{e_n}(E)) \le \diam(E).
\]

If \(v \in \R^n\) is a unit vector, the \emph{Steiner symmetrization of \(E\) with respect to the direction \(v\)} is obtained by choosing any orthogonal change of coordinate \(T \colon \R^n \to \R^n\) such that \(T(v)=e_n\) and letting
\[
    S_v(E) \coloneq T^{-1}(S_{e_n}(T(E))).
\]
Since \(T\) is an isometry, \(|S_v(E)|=|E|\) and \(\diam(S_v(E)) \le \diam(E)\).

\begin{prop}[Slicing perimeter by lines]\label{prop: slicing}
    Let \(E \subset \R^n\) be a Lebesgue measurable set with \(|E|<\infty\) and \(P(E)<\infty\). Then, 
    \begin{equation*}
        \int_{\R^{n-1}} P(E_z) \dif z \le P(E).
    \end{equation*}
    and in particular the vertical \(z\)-slice \(E_z\) is a set of finite perimeter in \(\R\) for a.e. \(z \in \R^{n-1}\).
\end{prop}
\begin{proof}
    Fix a mollifier \(\rho \in C^\infty_c(B_1)\) and let \(u_\eps \coloneq \chi_E \star \rho_\eps\), so that
    \(u_\eps \to \chi_E\) and 
...
\begin{thm}[Steiner inequality]\label{thm: steiner inequality}
    ...
    \begin{equation*}
        P(S_v(E)) \le P(E),
    \end{equation*}
    and the equality holds if and only the vertical \(z\)-slice \(E_z\) is an interval for a.e. \(z \in \R^{n-1}\). 

    In particular, if \(E\) is equivalent to a convex set, then the equality holds if and only if \(E\) is equivalent to \(S_v(E) + cv\) for some \(c \in \R\).
\end{thm}

\begin{proof}
    Without loss of generality, assume that \(v=e_n\) and denote \(E^s \coloneq S_{e_n}(E)\). We divide the proof in three steps.
    ...
    \begin{aligned}
        P(E) &= \Haus^{n-1}(\de E) = \sum_{h=1}^M\Haus^{n-1}(\de E \cap G_h \times \R)2 \\
    ...
    Let us denote
    \[
        D \coloneq \{ z \in \R^{n-1} \colon E_z \text{ is not an interval}\} = \bigcup \{ G_h \colon N(h) \ge 2\}.
    \]
    We have just found that
    \begin{align*}
        P(E)-P(E^s) &\ge \sum_{h=1}^M \int_{G_h \cap D} \left(\sqrt{4N(h)^2+|\nabla m|^2} - \sqrt{4+|\nabla m|^2} \right)\dif z\\
        &= \sum_{h=1}^M \int_{G_h \cap D} \frac{4(N(h)^2-1)}{\sqrt{4N(h)^2+|\nabla m|^2} + \sqrt{4+|\nabla m|^2}}\dif z\\
        & \ge 2 \frac{1}{\sqrt{4N(h)^2 + |\nabla m|^2}} \dif z.
    \end{align*}
    Thus, by Cauchy-Schwarz inequality and \eqref{eq: P(E^s)<(int)<P(E)},
    \begin{equation}\label{eq: estimate on D}
        \begin{aligned}
            2\Leb^{n-1}(D)^2 &= 2 \left( \int_D \frac{(4N(h)^2+|\nabla m|^2)^{1/4}}{(4N(h)^2+|\nabla m|^2)^{1/4}} \right)^2 \\
            &\ge P(E) (P(E)-P(E^s)).
        \end{aligned}
    \end{equation}
    ...
    \begin{equation}\label{eq: step 1 to E_h}
        P(E_h^s) \le P(E_h^s), \qquad 2 \Leb^{n-1}(D_h) \le P(E_h)(P(E_h)-P(E_h^s)).
    \end{equation}
    ...
    \begin{equation*}
    \begin{aligned}
        \int_G P(E_z) \dif z & \le \int_{\R^n} \liminf \chi_{G_h \setminus D_h}(z)P((E_h)_z) \dif z\\
    ...
    \emph{Step 3}. Assume that \(E\) is convex, \(|E|<\infty\) and \(P(E)=P(E^s)\). Since \(\Leb^n(\de E)=0\)\footnote{
    ...
    we may assume that \(E\) is open. Then the projection \(G\subset \R^{n-1}\) is a convex set, and for every \(z \in G\), the vertical \(z\)-slice is
    \[
        E_z= (-v(z),u(z)),
    \]
    where \(v,u \colon G \to \R\) are respectively convex and concave functions, so in particular they are locally Lipschitz and \(E\) has Lipschitz boundary. By Rademacher's theorem, \(\nabla u\) and \(\nabla v\) are defined at a.e. \(z \in G\), so
    \begin{equation*}%\label{eq: P(E), P(E^s) convex case}
    \begin{aligned}
        P(E) &= \int_G \left( \sqrt{1+|\nabla u|^2} + \sqrt{1+|\nabla v|^2} \right) \dif z\\
        P(E^s) &= \int_G  \sqrt{1+\left| \frac{\nabla u + \nabla v}{2}\right|^2} \dif z
    \end{aligned}
    \end{equation*}
    since 
    \[  
    ...
    \]
    Since the function \(z \mapsto \sqrt{1+|z|^2}\) is strictly convex,
    \[
        \sqrt{1+|\nabla u|^2} + \sqrt{1+|\nabla v|^2} \ge \sqrt{1+\left| \frac{\nabla u + \nabla v}{2}\right|^2}
    \]
...
\]
the set of with density one points of \(E\). A standard application of Besicovitch's covering theorem shows that \(|E \triangle E^{(1)}|=0\). 
...
\begin{proof}
    ...
    \end{aligned}        
    \]
    Now let \(z \in G_1 \cap G_2\). Since \(E_z\) is equivalent to an interval and \(\Leb^1(E_z \cap I_r(t_k))>0\) for \(k=1,2\), and since \eqref{eq: x_k density one}, we have
    \[
