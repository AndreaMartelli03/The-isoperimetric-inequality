...
\begin{proof}
    ...
    \]
    so \((E_h)_z \to E_z\) for a.e. \(z \in \R^{n-1}\), and by the lower semicontinuity of the perimeter
    ...
    Now, assume that \(P(E)=P(E^s)\) and let us prove that \(E_z\) is equivalent to an interval for a.e. \(z \in \R^{n-1}\). By Proposition~\ref{prop: slicing}, \(P(E_z)<\infty\) for a.e. \(z \in \R^{n-1}\). Moreover, since \(E_h \to E\), we have that
    \begin{equation*}
        G_h \to G.
    \end{equation*}
    ...
    Then, by \eqref{eq: step 1 to E_h}, \(\chi_{D_h} \to 0\) in \(L^1(\R^{n-1})\), so, up to passing to a subsequence, we have the pointwise limit
    \[
    ...
    \end{aligned}
    \end{equation*}
    having used that for every \(z \in G_h \setminus D_h\) the slice \((E_h)_z\) is equivalent to a bounded interval, and the 1-dimensional isoperimetric inequality, i.e., that \(P(E_z) \ge 2\) for a.e. \(z \in \R^{n-1}\). Since \(P(E_z) < \infty \) for a.e. \(z \in \R^{n-1}\), this implies that \(P(E_z)=2\) for a.e. \(z \in \R^{n-1}\), so, by the characterization of the equality in the 1-dimensional case, \(E_z\) is an interval for a.e. \(z \in \R^{n-1}\). 
\end{proof}
