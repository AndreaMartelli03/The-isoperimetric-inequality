Let us recall the definition of Steiner symmetrization. Let \(E \subset \R^n\) and fix a unit vector \(v\in \R^n\). For every \(z \in \langle v \rangle^\perp= \R^{n-1}\), consider the \emph{\(v\)-vertical \(z\)-slice}
\[
...
\]
In what follows, we will always identify \(\langle v \rangle^\perp = \R^{n-1}\) with a suitable change of coordinates, that is, we take \(v = e_n\). The usual notation will then be to write \(\R^n = \R^{n-1}\times \R\) with coordinates \(x=(z,t)\), \(z=(x_1,\dots,x_{n-1}) \in \R^{n-1}\), \(t=x_n \in \R\).
...
\begin{proof}
    Without loss of generality, assume that \(v=e_n\) and denote \(E_z=E_z^v\), \(E^s \coloneq S_{e_n}(E)\). We divide the proof into three steps.
    
    \emph{Step 1}. Assume that \(E\) is a bounded set with polyhedral boundary \(\de E\), and that the outer unit normal \(\mathbf{n}_{\de E}\) of \(E\) is never orthogonal to \(e_n\). In particular, \(P(E)<\infty\) and \(|D\chi_E| = \Haus^{n-1}\resmes \de E\). Let \(G\) be the \emph{essential projection} of \(E\) on \(\R^{n-1}\), that is,
    \[
        G \coloneq \{ z \in \R^{n-1} \colon \Leb^1(E_z)>0\}.
    \]
    Since \(\mathbf{n}_{\de E}\) is never orthogonal to \(e_n\), in the definition of set with polyhedral boundary, we can always take \(T\) to be a translation, so there exist a finite partition \(\{G_h\}_{h=1}^M\) of \(G\) made of \((n-1)\)-polyhedral sets and affine functions 
    \[
    ...
    \end{aligned}
    \end{equation*}
    (see Figure~\ref{fig: polyhedral set}).
    \begin{figure}[ht]
		\centering
		\begin{tikzpicture}
		...
		\end{tikzpicture}
        \caption{Notation adopted in step 1 of the proof of Theorem~\ref{thm: steiner inequality}.}
    ...
    \]
    is a bounded set with polyhedral boundary, and in particular it has finite perimeter and \(|D\chi_{E^s}|= \Haus^{n-1} \resmes (\Gamma(m/2,G) \cup \Gamma(-m/2,G))\) given by
    ...
    By \eqref{eq: m(z)} and the convexity of the function \(z \mapsto \sqrt{1+|z|^2}\),
    \begin{align*}
        \sum_{k=1}^{N(h)} \sqrt{1+|\nabla v_h^k|^2} + \sqrt{1+|\nabla u_h^k|^2} &\ge 2N(h) \sqrt{ 1+ \left|\sum_{k=1}^{N(h)} \frac{\nabla u_h^k-\nabla v_h^k}{2N(h)}\right|^2}\\
        &= \sqrt{4N(h)^2+|\nabla m|^2}
    \end{align*}
    ...
    \begin{equation}\label{eq: P(E^s)<(int)<P(E)}
        P(E^s) \le \sum_{h=1}^{M} \int_{G_h} \sqrt{4N(h)^2+|\nabla m|^2} \dif z \le P(E).
    \end{equation}

    We have just found that
    \begin{align}\label{eq: P(E)-P(E^s)}
    ...
    \]
    then by Cauchy-Schwarz inequality, \eqref{eq: P(E^s)<(int)<P(E)} and \eqref{eq: P(E)-P(E^s)},
    ...
    \]
    Note that we can assume that the outer unit normals \(\mathbf{n}_{\de E_h}\) of \(E_h\) are never orthogonal to \(e_n\). Indeed, as explained in Remark~\ref{rmk: unit normal of sets with polyhedral boundary}, each \(\mathbf{n}_{\de E_h}\) assumes finitely many values of \(\Sp^{n-1}\), so if it happens that \(\mathbf{n}_{\de E_h} \perp e_n\) somewhere, we only need to rotate slightly \(\de E\). Doing smaller and smaller rotations as \(h \to \infty\), we still have that \(E_h \to E\) (while \(P(E_h) \to P(E)\) isn't affected by rotations). 
    
    Observe that, since \((E_h \triangle E)_z=((E_h)_z\triangle E_z)\), by Fubini
    \[
        \int_{\R^{n-1}} \Leb^1((E_h)_z \triangle E_z) \dif z = |E_h \triangle E| \to 0,
    \]
    so \((E_h)_z \to E_z\) for a.e. \(z \in \R^{n-1}\), and by lower semicontinuity of the perimeter
    ...
    \end{equation}
    Since \(E_h \to E\), we have that
    \begin{equation}\label{eq: G_h -> G}
        G_h \to G.
    \end{equation}
    Indeed, \(G_h \triangle G\) is equivalent to
    \[
        \{ z \in \R^{n-1} \colon \Leb^1((E_h)_z \triangle E_z) >0\} = \bigsqcup_{k \in \Z} G_h^k,
    \]
    where
    \[
        G_h^k \coloneq \left\{ z \in \R^{n-1} \colon \left(1+\frac{1}{h}\right)^k \le \Leb^1((E_h)_z \triangle E_z) < \left(1+\frac{1}{h}\right)^{k+1} \right\}.
    \]
    Then, 
    \begin{align*}
        \sum_{k \in \Z} \left(1+\frac{1}{h}\right)^k \Leb^{n-1}(G_h^k) \le \sum_{k \in \Z}\int_{G_h^k} \Leb^1((E_h)_z \triangle E_z) \dif z = |E_h \triangle E|,
    \end{align*}
    and taking the limsup as \(h \to 0\), we get \(\limsup_{h \to \infty}\Leb^{n-1}(G_h \triangle G) = 0\).
    
    Let us prove that \(P(E^s) \le P(E)\). By Fubini's theorem,
    \begin{equation*}
    \begin{aligned}
        |E_h^s \triangle E^s| &= \int_{\R^{n-1}}|m_h(z)- m(z)| \dif z \\
            &\le \int_{\R^{n-1}} \Leb^1((E_h)_z \triangle E_z) \dif z =|E\triangle E_h|
    \end{aligned}
    \end{equation*}
    having used that \(|\Leb^1(A)-\Leb^1(B)| \le \Leb^1(A\triangle B) \) for every \(A,B \subset \R\). In particular \(E_h^s \to E^s\) and by lower semicontinuity
    \[
        P(E^s) \le \liminf_{h \to \infty} P(E_h^s) \le \lim_{h \to \infty} P(E_h) = P(E).
    \]

    Now, assume that \(P(E)=P(E^s)\) and let us prove that \(E_z\) is equivalent to an interval for a.e. \(z \in \R^{n-1}\). By \eqref{eq: step 1 to E_h} and \eqref{eq: G_h -> G}, \(\chi_{D_h} \to 0\) and \(\chi_{G_h} \to \chi_G\) in \(L^1(\R^{n-1})\), so, up to passing to a subsequence, we have the pointwise limit
    \[
        \chi_{G}(z) = \lim_{h \to \infty}\chi_{G_h\setminus D_h}(z)
    \]
    for a.e. \(z \in \R^{n-1}\). Combining this with \eqref{eq: P(E_z) < liminf P(E_hz)},
    \[
    ...
    \end{aligned}
    \end{equation*}
    having used that for every \(z \in G_h \setminus D_h\), the slice \((E_h)_z\) is equivalent to a bounded interval, so \(P(E_z)=2\). 
    But by the 1-dimensional isoperimetric inequality, \(P(E_z) \ge 2\) for a.e. \(z \in \R^{n-1}\), so 
    \[
        \int_GP(E_z) \dif z \le 2\Leb^{n-1}(G) \le \int_G P(E_z),
    \]
    and this implies that \(P(E_z)=2\) for a.e. \(z \in G\). By the characterization of the equality in the 1-dimensional isoperimetric inequality, \(E_z\) is an interval for a.e. \(z \in G\). If \(z \in \R^n \setminus G\), \(\Leb^1(E_z)=0\), so \(E_z\) is equivalent to the empty set (which is trivially an interval).
\end{proof}
