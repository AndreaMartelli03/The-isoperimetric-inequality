...
\begin{thm}[Steiner inequality]\label{thm: steiner inequality}
    ...
    Moreover, if \(E\) is equivalent to a convex set, then the equality holds if and only if \(E\) is equivalent to \(S_v(E) + cv\) for some \(c \in \R\).
\end{thm}
...
\begin{proof}
    ...
    \begin{equation}\label{eq: step 1 to E_h}
        P(E_h^s) \le P(E_h^s), \qquad 2 \Leb^{n-1}(D_h)^2 \le P(E_h)(P(E_h)-P(E_h^s)).
    \end{equation}
